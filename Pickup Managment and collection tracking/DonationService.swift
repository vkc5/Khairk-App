...
final class DonationService {
    ...
    func listenPendingDonations(ngoId: String, onChange: @escaping (Result<[Donation], Error>) -> Void) -> ListenerRegistration {
    ...
    func listenActivePickups(ngoId: String, onChange: @escaping (Result<[Donation], Error>) -> Void) -> ListenerRegistration {
        db.collection("donations")
            .whereField("ngoId", isEqualTo: ngoId)
    ...
    func listenDonation(donationId: String, onChange: @escaping (Result<Donation, Error>) -> Void) -> ListenerRegistration {
        db.collection("donations").document(donationId)
            .addSnapshotListener { doc, err in
                if let err = err { onChange(.failure(err)); return }
                guard let doc = doc, let d = Donation(doc: doc) else {
                    onChange(.failure(NSError(domain: "DonationService", code: 404, userInfo: [NSLocalizedDescriptionKey: "Donation not found"])))
                    return
                }
                onChange(.success(d))
            }
    ...
    // Approve donation: set donation approved + pickup in_progress + increment case collected
    func approveDonation(ngoId: String, donationId: String, caseId: String, quantity: Int, completion: @escaping (Result<Void, Error>) -> Void) {
